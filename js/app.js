/* ============================================
   Sleep Academy - Main Application
   ============================================ */
...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('sleep-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('sleep-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('sleep-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('sleep-progress', JSON.stringify(this.progress));
        localStorage.setItem('sleep-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        var html = '<div class="fade-in">' +
            ...
            '<h2>睡眠 Academy へようこそ</h2>' +
            '<p>睡眠の科学と最高の眠りを手に入れる技術を体系的に学べるプラットフォームです。</p>' +
            '</div>' +
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('sleep-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('sleep-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            ...
            localStorage.removeItem('sleep-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
